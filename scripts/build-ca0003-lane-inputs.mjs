...
const selection = JSON.parse(
  fs.readFileSync(path.join(dataDir, "DALILI_CA_0003_SELECTION_01.json"), "utf8"),
);
...
for (const lane of ["A", "B", "C"]) {
  ...
  if (records.length !== 100) throw new Error(`CA-0003 lane ${lane} must contain 100 sources`);
  const artifact = {
    artifact: `DALILI_CA_0003_LANE_${lane}_INPUT_01`,
    version: "1.0",
  ...
  fs.writeFileSync(
    path.join(dataDir, `DALILI_CA_0003_LANE_${lane}_INPUT_01.json`),
    `${JSON.stringify(artifact, null, 2)}\n`,
  );
}

process.stdout.write(`${JSON.stringify({ batch: "CA-0003", lanes: { A: 100, B: 100, C: 100 } }, null, 2)}\n`);
