...
## 7. اختبارات القبول (منفّذة)
| الاختبار | النتيجة |
|---|---|
| typecheck + production build | ناجح |
| تصنيف كل الفرص (1413) بلا قيم فارغة أو غير صالحة | ناجح |
| `/activities` → «ابدأ الآن» → `/activities/level` | ناجح |
| اختيار مستوى → `/activities/options?level=` → `/activities/browse?level=&view=` | ناجح |
| عرض المجالات مفلتراً بالمستوى وفتح البطاقة كاملة | ناجح (مثال: «شراء ملابس جديدة للأسرة» — متقدم) |
| القائمة العامة والبحث داخلها | ناجح (بسيط: 226 فرصة منشورة، بحث «الملابس» = 3) |
| أحداث اليوم مفلترة بالمستوى | ناجح |
| مستوى غير صالح في الرابط → «جميع المستويات» دون كسر | ناجح |
| ظهور شارة المستوى داخل بطاقة المشاركة | ناجح |
| المسارات القديمة (`/activities/home`, `/activities/my-day`, `/search`) | تعمل دون تغيير |
| أخطاء console | 0 |

ملاحظة: الأعداد الظاهرة في الواجهة تعكس الفرص ذات البطاقات المكتملة والمنشورة فقط، بينما التصنيف مطبّق على كامل السجل (1413).
